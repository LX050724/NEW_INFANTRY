...
//    mpu_data.gz = ((mpu_buff[12] << 8 | mpu_buff[13]) - mpu_data.gz_offset);
		...
		mpu_data.gx= diff_convert(((mpu_buff[8]  << 8 | mpu_buff[9])  - mpu_data.gx_offset),&Diff_GX);
		mpu_data.gy= diff_convert(((mpu_buff[10] << 8 | mpu_buff[11]) - mpu_data.gy_offset),&Diff_GY);
		mpu_data.gz= diff_convert(((mpu_buff[12] << 8 | mpu_buff[13]) - mpu_data.gz_offset),&Diff_GZ);
	
//		mpu_data.gx *= 0.125f;
//	  mpu_data.gy *= 0.125f;
//	  mpu_data.gz *= 0.125f;
//		mpu_data.gz = KalmanFilter(&filter_y,((mpu_buff[12] << 8 | mpu_buff[13]) - mpu_data.gz_offset));
...
void mpu_offset_call(void)
{
	...
	do
	{
		...
		for(i=0;i<20;i++)
		{
			mpu_get_data();
			imu_ahrs_update();
			Fix_data[0][i] = imu.yaw;
			Fix_data[1][i] = 1/(TIM7->CNT/10000.0f);
			MPU_DELAY(1);
		}
		arm_sub_f32(&Fix_data[0][1],Fix_data[0],Fix_sub,19);
		arm_mult_f32(Fix_sub,&Fix_data[1][1],Fix_mult,19);
		arm_mean_f32(Fix_mult,19,&Fix);
	}while((Fix < -10)||(Fix > 10));
}
...
//	last_update = now_update;
	...
	if(imu.yaw - imu.yawlast < -180)imu.rand++;
	if(imu.yaw - imu.yawlast > 180)imu.rand--;
	imu.realyaw = imu.rand*360 + imu.yaw;
	imu.yawlast = imu.yaw;
	imu.realyaw-=Fix*halfT*2;
...
{
	/* yaw    -pi----pi */
	imu.yaw = -atan2(2*q1*q2 + 2*q0*q3, -2*q2*q2 - 2*q3*q3 + 1)* 57.295779513079f; 
	/* pitch  -pi/2----pi/2 */
	imu.pit = -asin(-2*q1*q3 + 2*q0*q2)* 57.295779513079f;   
	/* roll   -pi----pi  */	
	imu.rol =  atan2(2*q2*q3 + 2*q0*q1, -2*q1*q1 - 2*q2*q2 + 1)* 57.295779513079f;
}
