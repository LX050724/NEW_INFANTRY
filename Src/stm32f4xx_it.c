...
void USART1_IRQHandler(void)
{
  ...
  if ((__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE) != RESET) && __HAL_UART_GET_IT_SOURCE(&huart1, UART_IT_IDLE) != RESET)
  {
    ...
    HAL_UART_Receive_DMA(&huart1, usart1_dma_buff, BUFLEN);
		...
		chick = ((int16_t)usart1_dma_buff[0] | ((int16_t)usart1_dma_buff[1] << 8)) & 0x07FF;
		
		if((chick > 364)&&(chick < 1684))
		{
...
void USART3_IRQHandler(void)
{
  ...
  if ((__HAL_UART_GET_FLAG(&huart3, UART_FLAG_IDLE) != RESET) && __HAL_UART_GET_IT_SOURCE(&huart3, UART_IT_IDLE) != RESET)
  {
    ...
    /*发送消息到队列尾*/
    xQueueSendToBackFromISR(referee_system_Queue, usart3_dma_buff, &xHigherPriorityTaskWoken);

    HAL_UART_Receive_DMA(&huart3, usart3_dma_buff, USART3_BUFLEN);
    portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
  }
#ifdef huart3IRQHandler
...
{
  if ((GPIO_Pin == GPIO_PIN_8)|(GPIO_Pin == GPIO_PIN_1))
  {
    BaseType_t xHigherPriorityTaskWoken;

    xSemaphoreGiveFromISR(IMU_SemaphoreHandle, &xHigherPriorityTaskWoken);
    portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
  }
}
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  {
    CAN1_Receive_Msg(can1_Receive_buf);
    switch (CAN_Rx.StdId)
    {
			case 0x201:
				RM3510_Actual_Speedt.Chassis_Motor_Actual_Speed_1 = RM3510_Feedback(can1_Receive_buf);
				break;
			case 0x202:
				RM3510_Actual_Speedt.Chassis_Motor_Actual_Speed_2 = RM3510_Feedback(can1_Receive_buf);
				break;
			case 0x203:
				RM3510_Actual_Speedt.Chassis_Motor_Actual_Speed_3 = RM3510_Feedback(can1_Receive_buf);
				break;
			case 0x204:
				RM3510_Actual_Speedt.Chassis_Motor_Actual_Speed_4 = RM3510_Feedback(can1_Receive_buf);
				break;
